...
class Client
{
    ...
    public function run() {
        ...
        socket_sendto($socket, $this->message, strlen($this->message),0, $this->serverIp, $this->serverPort);

        socket_recvfrom($socket,$buffer,self::MAX_RECV_LEN,0,$serverIp,$serverPort);
        $this->printAcknowledgement($buffer, $serverIp, $serverPort);
    }

    /**
     * Prints the message received from the server + server details.
     *
     * @param $buffer
     * @param $serverIp
     * @param $serverPort
